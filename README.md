...
* **实时终端监控**：深度监听 `~/.local/share/fish/fish_history`，通过 `tail -f` 机制实现零延迟的数据捕获。
* **智能指令过滤**：自动忽略 `ls`, `cd`, `pwd`, `exit` 等日常无意义的导航指令，只打印真正具有记录价值的操作。
* **蓝牙长连接与自愈**：
    * **断线重连**：内置强大的 `_ensure_connection` 逻辑，设备断电或超出范围后，恢复时可自动重新建立连接。
    * **硬件保活机制**：针对带有自动关机机制的便携打印机，程序每隔 180 秒会自动发送一次静默通讯（如 CUT 指令），重置打印机休眠倒计时，确保全天候待命。
